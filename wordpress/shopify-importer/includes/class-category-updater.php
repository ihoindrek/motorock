...
class Shopify_Importer_Category_Updater {
    ...
    public function __construct($site) {
        ...
        $this->category_resolver = new Shopify_Importer_Category_Resolver($site);
    ...
    public function update_batch($page = 1, $batch = 0, $session_key = '') {
        ...
        if (empty($session_key)) {
            $session_key = 'shopify_categories_' . $this->site_id . '_' . wp_generate_password(8, false);
        }

        $session = $this->get_session($session_key);

        if ($page === 1 && $batch === 0 && empty($session['started'])) {
            $logger = new Shopify_Importer_Logger($this->site_id . '-categories');
            $session = array(
                'started' => true,
                'pages' => array(),
                'stats' => array(
            ...
            $this->logger = $logger;
            $this->log_line('Starting category update for site: ' . $this->site['name']);
        } else {
            $log_file = isset($session['log_file']) ? $session['log_file'] : '';
            $this->logger = new Shopify_Importer_Logger($this->site_id . '-categories', $log_file);
        }
        ...
        foreach ($batch_products as $shopify_product) {
            $stats['processed']++;
            try {
                $this->process_product_categories($shopify_product, $stats);
            } catch (Throwable $e) {
                $stats['skipped']++;
        ...
        if (!$has_more) {
            $this->log_line(
                'Category update complete. Updated: ' . $stats['updated'] .
                ', Unchanged: ' . $stats['unchanged'] .
                ', Skipped: ' . $stats['skipped']
            );

            Shopify_Importer_Site_Manager::update_site($this->site_id, array(
                'last_category_update_at' => current_time('mysql'),
                'last_category_update_stats' => $stats,
            ));
    ...
    private function process_product_categories($shopify_product, &$stats) {
        ...
        $product_id = $this->find_wc_product_id($shopify_product, $product_data);

        if (!$product_id) {
            $stats['skipped']++;
            $this->log_line('Skipped ' . $shopify_product['title'] . ' (no matching SKU in WooCommerce)', 'SKIP');
            return;
        }

        $result = $this->category_resolver->resolve($collection_ids);
        if (empty($result['category_ids'])) {
            $stats['skipped']++;
            $this->log_line('Skipped ' . $shopify_product['title'] . ' (no mapped categories)', 'SKIP');
            return;
        }

        $new_ids = array_map('intval', $result['category_ids']);
        $changed = Shopify_Importer_WPML_Helper::sync_product_categories($product_id, $new_ids);
        ...
        $label = $this->category_resolver->get_category_label($new_ids);
        if ($result['used_default']) {
            $this->log_line('Updated ' . $shopify_product['title'] . ' → ' . $label . ' (default)', 'OK');
        } else {
            $this->log_line(
                'Updated ' . $shopify_product['title'] . ' → ' . $label . ' via ' . implode(', ', $result['sources']),
                'OK'
            );
        }
    ...
    private function find_wc_product_id($shopify_product, $product_data) {
        if ($product_data) {
            $product_id = $this->find_existing_product_id($product_data);
            if ($product_id) {
                return $this->normalize_product_id($product_id);
            }
        }
        ...
        foreach ($shopify_product['variants'] as $variant) {
            ...
            $product_id = $this->get_product_id_by_sku($variant['sku']);
            if ($product_id) {
                return $this->normalize_product_id($product_id);
            }
        }
