...
class Motorock_Ai_Rest_Write {
	...
	public static function register_routes() {
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( __CLASS__, 'handle_write_post' ),
				'permission_callback' => array( __CLASS__, 'verify_secret' ),
			)
		);
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( __CLASS__, 'handle_write_related' ),
				'permission_callback' => array( __CLASS__, 'verify_secret' ),
			)
		);
	...
	public static function handle_write_post( WP_REST_Request $request ) {
		...
		Motorock_Ai_Logger::info( 'blog write request received', array( 'slug' => $payload['slug'] ?? null ) );

		$result = Motorock_Ai_Blog_Writer::write( $payload );
	...
	public static function handle_write_related( WP_REST_Request $request ) {
		$payload = $request->get_json_params();
		if ( ! is_array( $payload ) ) {
			return new WP_Error( 'motorock_ai_invalid_body', 'Invalid JSON body', array( 'status' => 400 ) );
		}
		...
		Motorock_Ai_Logger::info(
			'related products write request received',
			array( 'productId' => $payload['productId'] ?? null )
		);

		$result = Motorock_Ai_Related_Writer::write( $payload );
		if ( is_wp_error( $result ) ) {
			return $result;
		}
